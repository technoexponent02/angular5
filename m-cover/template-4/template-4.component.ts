...
export class Template4Component implements OnInit {
  ...
  templateModel: any = {
    ...
    headerColorOne: '#343436',
    headerImageOne: {},
    headerColorTwo: '#343436',
    headerImageTwo: {},
    headerColorThree: '#343436',
    headerImageThree: {},
  };
  /**
  ...
  /**
   * @func {{profileUpload}}
   * @description {{change the upload image status and file upload component status}}
   */
  profileUpload() {
   ...
   */
  headerImageUploadOne() {
    this.uploadImage.status = 'header-image-tpl1';
    this.uploadImage.width = 825;
   ...
   * @description {{change the upload image status and file upload component status for the header image}}
   */
  headerImageUploadTwo() {
    this.uploadImage.status = 'header-image-tpl2';
    this.uploadImage.width = 825;
    this.uploadImage.height = 335;
    this.uploadComponentShow = true;
  }
   ...
   * @description {{change the upload image status and file upload component status for the header image}}
   */
  headerImageUploadThree() {
    this.uploadImage.status = 'header-image-tpl3';
    this.uploadImage.width = 825;
    this.uploadImage.height = 335;
    this.uploadComponentShow = true;
  }
  ...
  receiveUploadedDataAndState($event) {
     ...
     * @description {{this will set the upload profile image data }}
     */
    if ($event.imagesource.image !== undefined && $event.whichImage == 'header-image-tpl1'){
      this.templateModel.headerImageOne = $event;
    }
    /**
     * @description {{this will set the upload profile image data }}
     */
    if ($event.imagesource.image !== undefined && $event.whichImage == 'header-image-tpl2'){
      this.templateModel.headerImageTwo = $event;
    }
    /**
     * @description {{this will set the upload profile image data }}
     */
    if ($event.imagesource.image !== undefined && $event.whichImage == 'header-image-tpl3'){
      this.templateModel.headerImageThree = $event;
    }
