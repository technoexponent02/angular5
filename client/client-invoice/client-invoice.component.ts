...
export class ClientInvoiceComponent implements OnInit {
  ...
  caltoDate: any = '';
  /**
   * @property \{{{type}}\} {{queryObj}} {{this will hold the query object data from the url}}
   */
  queryObj: any = '';
  /**
   * @property \{{{any}}\} {{userId}} {{this will take the user id}}
   */
  userId: any = '';
  /**
   * @property \{{{any}}\} {{userToken}} {{this will take the user token data}}
   */
  userToken: any = '';
  ...
  frmCall($data){
    ...
    if($data == ''){
      ...
      this.getAllInvoiceData(this.userToken, this.userId, this.calfrmDate, this.caltoDate);
    }
    // console.log('fire ajax with - ', `${this.calfrmDate} - ${this.caltoDate}` );
    // get all data as per filter date range
    this.getAllInvoiceData(this.userToken, this.userId, this.calfrmDate, this.caltoDate);
          ...
          if(data.status === "success"){
            ...
            this.invoiceData = invoice.map(invoice => (
              { 
                ...
                link: config.api_base_url+'invoicePdf?api_token='+this.userToken+'&'+invoice.invoice_link
              }
            ));
  ...
  ngOnInit() {
    ...
    this.route.queryParamMap.subscribe(params => {
      ...
      // set user the token
      this.userToken = this.queryObj.params.token;
      // set the user id
      this.userId = JSON.parse(sessionStorage.getItem('clientUserId'));
